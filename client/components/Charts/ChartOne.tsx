...
import { AppointmentData } from '@/types/interface'
...
const ChartOne: React.FC = () => {
  ...
  const fetchAppointmentsData = async () => {
    try {
      // TODO: Replace mock data with real API when backend is ready
  ...
  // Memoize month data to prevent recreation on every render
  const monthData = useMemo(() => ({
    ...
    series: [
      {
        name: `Tìm kiếm Năm ${selectedYear}`,
  ...
  }), [selectedYear, appointmentsData, chartTypeLine]);

  const yearData = useMemo(() => ({
    categories: years.map((year) => year.toString()),
    series: [
      {
        name: 'Thống kê tỉ lệ tìm kiếm',
        data: years.map((year) => {
    ...
    chartType: chartTypeLine,
    yMax: Math.max(100, ...appointmentsData.map((a) => a.price)),
  }), [years, appointmentsData, chartTypeLine]);
        ...
        <div className="flex w-full flex-wrap gap-3 sm:gap-5">
          <div className="flex min-w-47.5">
            ...
            <div className="w-full">
              <p className="font-semibold text-[#6577F3]">Tìm kiếm</p>
